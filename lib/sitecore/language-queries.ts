export const GET_INSTANCE_LANGUAGES_QUERY = `
  query ContentPilotInstanceLanguages {
...
export const SITE_ROOT_IN_LANGUAGE_QUERY = `
  query ContentPilotSiteRootInLanguage($path: String!, $language: String!) {
...
export const ITEM_VERSION_STRICT_PROBE_QUERY = `
  query ContentPilotItemVersionStrictProbe($path: String!, $language: String!) {
    item(where: { path: $path, language: $language }) {
      itemId
...
export const ADD_ITEM_VERSION_MUTATION = `
  mutation ContentPilotAddItemVersion($input: AddItemVersionInput!) {
